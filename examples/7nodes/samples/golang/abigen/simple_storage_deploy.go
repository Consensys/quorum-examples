...
func main() {
	...
	simplestorage, transaction, s, err := storagecontract.DeploySimplestorage(trOpts, ethClient, big.NewInt(123))
	...
	println("contractAddress: " + simplestorage.Hex())
	println("transactionHash: " + transaction.Hash().Hex())

	// you need to wait until the submitted transaction is minted (you may opt to poll for the transaction receipt instead of sleeping)
